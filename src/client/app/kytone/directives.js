(function() {
    ...
    return {
      ...
      link: function(scope, element, attr) {
        $timeout(function() {
          ...
          var maxWitdh =
            $('#plPlace')[0].clientWidth - $('.track-time')[0].clientWidth;
  ...
  function titleUpdate(socket, Playlist) {
    return {
      restrict: 'A',
      link: function(scope, element, attr) {
    ...
    return {
      ...
      link: function(scope, element, attr) {
        ...
        if (
          ...
          localStorageService.get('volume') !== null
        ) {
  ...
  function playerStatus($interval, localStorageService) {
    return {
      ...
      link: function(scope, element, attr) {
        ...
        if (
          localStorageService.isSupported &&
          localStorageService.get('isPlaying') !== null
        ) {
